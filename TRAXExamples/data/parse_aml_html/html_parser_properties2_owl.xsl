<xsl:stylesheet version="1.0"
			...
			<!-- Target aml_html_properties_p1.xml -->
<xsl:output method="xml" indent="yes"/>
<xsl:strip-space elements="*"/>
<xsl:template match="/">  
            <xsl:apply-templates select="amlpropertyinfo"/>
</xsl:template>

<xsl:template match="amlpropertyinfo">
...
<!-- The ns-declarations in stylesheet root element are also copied to the result xml -->
	 <rdf:RDF xmlns="http://data.ifs.tuwien.ac.at/aml/ontology#" xml:base="http://data.ifs.tuwien.ac.at/aml/ontology">
	 	<xsl:comment>VPA: HTML2OWL Translation result using html_parser_properties2_owl.xsl</xsl:comment>
		<owl:Ontology rdf:about="http://data.ifs.tuwien.ac.at/aml/ontology"/>	
		<xsl:apply-templates select="div/div[@class='term']"/>	
	</rdf:RDF>
</xsl:template>
...
<xsl:template match="span[@rel='rdfs:domain']">
...
<xsl:template match="span[@rel='rdfs:range']">
