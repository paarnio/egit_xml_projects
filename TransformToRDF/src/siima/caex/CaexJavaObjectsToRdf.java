package siima.caex;

import java.io.File;
import java.io.FileOutputStream;
import java.io.IOException;
import java.io.InputStream;
import java.net.MalformedURLException;
import java.net.URL;
import java.util.Collection;
import java.util.LinkedList;

import javax.xml.bind.JAXBContext;
import javax.xml.bind.JAXBException;
import javax.xml.bind.Unmarshaller;

import net.semwebprogramming.chapter9.JavaObjectRdfSerializer;
import net.semwebprogramming.chapter9.weather.CurrentObservation;
import siima.models.jaxb.caex.CAEXFile;

public class CaexJavaObjectsToRdf {
	  /**
	    * Program that converts weather XML feed to RDF using
	    * a combination of JAXB and the Java object serializer
	    * @param args
	    *    input xml - The URL of the input Weather XML file
	    *       (http://www.weather.gov/xml/current_obs/KBWI.xml)
	    *    output file - the file name of the output RDF file
	    *       (ouputs in TURTLE format)
	    */
	   public static void main(String[] args)
	   {
		   /* orig. 
	      if(args.length != 2) 
	      {   
	         System.err.println("Usage: java WeatherJavaObjectsToRdf "
	            + "[input xml] [output file]");
	         return;
	      }

	      String xmlFile = args[0];
	      String outputFile = args[1];
	      */
		   
		   String xmlFile = "data/caex/caex_lego/Lego_example_mod2_cut.aml"; //"data/weather/KBWI.xml";
		   String outputFile = "data/caex/caex_lego/Lego_example_mod2_cut_output.rdf"; //"data/weather/KBWIoutput.rdf";
	      try
	      {
	         //get the xml file input
	    	 File myFile=new File(xmlFile);
	    	 
	         //URL xmlFileUrl = new URL(xmlFile);
	    	 URL xmlFileUrl = myFile.toURI().toURL();
	         InputStream xmlFileInputStream = xmlFileUrl.openStream();
	         
	         //create the output file
	         FileOutputStream outputStream = new FileOutputStream(outputFile);

	         //unmarshal the information from xml
	         JAXBContext jaxbContext = 
	            JAXBContext.newInstance("siima.models.jaxb.caex"); //"net.semwebprogramming.chapter9.weather"
	         Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
	         
	        // CurrentObservation currentObservation = 
	         //   (CurrentObservation)unmarshaller.unmarshal(xmlFileInputStream);
	         CAEXFile caexfile = 
	 	            (CAEXFile)unmarshaller.unmarshal(xmlFileInputStream);

	         JavaObjectRdfSerializer serializer = new JavaObjectRdfSerializer(
	               "http://siima.org/caex/individuals#", 
	               "http://siima.org/caex/ontology#", 
	               "TURTLE", 
	               "siima.models.jaxb.caex");
	         
	         Collection<Object> objects = new LinkedList<Object>();
	         objects.add(caexfile);
	         serializer.serialize(objects, outputStream);
	         outputStream.close();
	         
	         System.out.println("Success.");
	      }
	      catch(MalformedURLException e)
	      {
	         e.printStackTrace();
	      }
	      catch (IOException e)
	      {
	         e.printStackTrace();
	      }
	      catch (JAXBException e)
	      {
	         e.printStackTrace();
	      }
	      catch (Exception e)
	      {
	         e.printStackTrace();
	      }
	   }
}
